...
Adds production-grade password recovery and first-login enforcement without
replacing the legacy login/2FA implementation. Reset tokens are never persisted
in clear text: only their SHA-256 digest is stored in the persistent database.
"""
...
def _clean_reset_store(raw: object) -> dict:
    ...
    cleaned = {}
    for digest, item in source.items():
        if not isinstance(item, dict):
            continue
        if float(item.get("expires_at") or 0) <= now:
            continue
        cleaned[str(digest)] = dict(item)
    return cleaned
...
<div style="max-width:600px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb">
  ...
  <div style="padding:30px 28px">
    <h1 style="font-size:22px;margin:0 0 18px">Réinitialisation du mot de passe</h1>
    <p>Bonjour {safe_name},</p>
    <p>Une demande de réinitialisation de votre mot de passe a été effectuée pour votre compte d’administration BININGA.</p>
    ...
    <p style="font-size:13px;color:#667085">Pour votre sécurité, ne transférez pas cet email et ne partagez pas le lien de réinitialisation.</p>
  </div>
  <div style="padding:18px 28px;background:#f8fafc;color:#98a2b3;font-size:12px">Message automatique de sécurité — Administration BININGA</div>
</div></body></html>"""
...
def _send_reset_email(user: dict, reset_url: str) -> bool:
    ...
    if _send_via_resend(address, subject, html_body):
        return True
    return _send_via_smtp(address, subject, html_body)
...
def _handle_forgot(server, handler) -> bool:
    ...
    store = _load_reset_store(server)
    now = time.time()
    recent = any(
        isinstance(item, dict)
        and item.get("username") == user.get("username")
        and now - float(item.get("created_at") or 0) < RESET_COOLDOWN_SECONDS
    ...
    server.audit_log(
        ...
        f"Demande de réinitialisation pour {user.get('username')} — email {'envoyé' if delivered else 'non envoyé'}",
    )
    handler._json(generic)
    return False
...
def _handle_reset(server, handler) -> bool:
    ...
    if not token:
        handler._json({"ok": False, "message": "Lien de réinitialisation invalide ou expiré."}, 400)
        return False
    ...
    if not isinstance(item, dict) or float(item.get("expires_at") or 0) <= time.time():
        ...
        handler._json({"ok": False, "message": "Ce lien de réinitialisation est invalide ou a expiré."}, 400)
        return False
    ...
    handler._json({"ok": True, "message": "Votre mot de passe a été modifié. Vous pouvez maintenant vous connecter."})
    return False
...
def _handle_change(server, handler) -> bool:
    ...
    if not user or not server._verify_password(current_password, str(user.get("password_hash") or "")):
        handler._json({"ok": False, "message": "Le mot de passe actuel est incorrect."}, 400)
        return False
    ...
    if server._verify_password(new_password, str(user.get("password_hash") or "")):
        handler._json({"ok": False, "message": "Le nouveau mot de passe doit être différent de l’ancien."}, 400)
        return False
    ...
    handler._json({"ok": True, "reauthenticate": True, "message": "Mot de passe modifié. Reconnectez-vous avec votre nouveau mot de passe."})
    return False
...
def _handle_user_upsert(server, handler) -> bool:
    ...
    role = str(data.get("role") or "lecteur")
    password = str(data.get("password") or "").strip()
    email_address = _email(data.get("email"))
    if not username or role not in ALLOWED_ROLES:
        handler._json({"ok": False, "message": "Données utilisateur invalides."}, 400)
        return False
    if not _valid_email(email_address):
        handler._json({"ok": False, "message": "Une adresse email valide est obligatoire pour la récupération du compte."}, 400)
        return False
    ...
    if existing:
        ...
        if not password:
            handler._json({"ok": False, "message": "Un mot de passe provisoire est obligatoire."}, 400)
            return False
        error = _password_error(password, username)
        if error:
            handler._json({"ok": False, "message": error}, 400)
...
def _block_until_password_changed(server, handler) -> bool:
    path = _path(handler)
    if not path.startswith("/api/") or path in PASSWORD_CHANGE_PATHS or path in PUBLIC_AUTH_POSTS or path == "/api/login":
        return True
    token = str(handler.headers.get("X-Admin-Token", "") or "")
    session = server.get_session(token) if token else None
    if not session:
    ...
    if user and user.get("must_change_password"):
        handler._json({
            ...
            "must_change_password": True,
            "message": "Vous devez définir votre nouveau mot de passe avant d’accéder à l’administration.",
        }, 428)
        return False
    return True
...
def guard_request(server, handler):
    """Handle auth lifecycle routes and enforce first-login password changes."""
    path = _path(handler)
    ...
    if method == "POST" and path == "/api/auth/forgot-password":
        return _handle_forgot(server, handler)
    if method == "POST" and path == "/api/auth/reset-password":
        return _handle_reset(server, handler)
    if method == "POST" and path == "/api/auth/change-password":
        return _handle_change(server, handler)
    if method == "GET" and path == "/api/auth/session":
        return _handle_session(server, handler)
    if method == "GET" and path == "/api/auth/users-meta":
