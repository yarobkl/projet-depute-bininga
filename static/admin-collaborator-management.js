 ...
 * password, role and granular permissions they choose. Recovery email is optional.
 */
(() => {
  'use strict';
  ...
  function clarifyCollaboratorForm() {
    ...
    if (!email) return;
    const group = email.parentElement; const label = group?.querySelector('label');
    ...
    if (help) help.textContent = 'Facultatif pour les collaborateurs. Sans email, le mot de passe pourra être modifié par un Owner depuis cet espace.';
  }

  function permissionOverrides() {
    try {
      return window.BiningaPermissionEditor?.collect?.() || { permission_grants: [], permission_revokes: [] };
    } catch (_) {
  ...
  function installOwnerOnlySubmit() {
    ...
    const submit = async function submitOwnerManagedCollaborator() {
      const api = core();
      if (!api || !api.isMainAdmin()) return toast('Seuls les propriétaires peuvent créer ou modifier un collaborateur.', true);
      ...
      if (!/^[A-Za-z0-9._@+-]{3,80}$/.test(username)) return toast('Identifiant invalide : utilisez au moins 3 caractères.', true);
      if (!['admin', 'editeur', 'lecteur', 'ministre'].includes(role)) return toast('Rôle utilisateur invalide.', true);
      ...
      try {
        const response = await api.request('/api/users/upsert', {
          ...
          body: JSON.stringify({ username, nom: name, email, password, role, ...permissions }), cache: 'no-store',
        });
        const data = await response.json();
        if (!data.ok) return toast(data.message || 'Collaborateur non enregistré.', true);
        toast('Collaborateur enregistré avec les droits sélectionnés.');
        if (typeof window.BiningaPermissionEditor?.refresh === 'function') await window.BiningaPermissionEditor.refresh();
      ...
      } catch (_) { toast('Impossible d’enregistrer le collaborateur.', true); }
    };
    submit.__ownerCollaboratorModel = true; window.submitUserForm = submit;
  }
  ...
  function protectOwnerRows() {
    document.querySelectorAll('.user-item[data-owner="1"]').forEach(item => {
      item.querySelectorAll('button,[onclick]').forEach(control => {
        const text = String(control.textContent || '').toLowerCase(); const onclick = String(control.getAttribute?.('onclick') || '').toLowerCase();
  ...
  function wrapLoadUsers() {
    ...
    const wrapped = async function() {
      ...
      clarifyCollaboratorForm(); protectOwnerRows();
      if (typeof window.BiningaPermissionEditor?.refresh === 'function') await window.BiningaPermissionEditor.refresh();
      return result;
    };
    wrapped.__collabWrapped = true; window.loadUsers = wrapped;
  }

  function install(){ clarifyCollaboratorForm(); installOwnerOnlySubmit(); wrapLoadUsers(); protectOwnerRows(); }
