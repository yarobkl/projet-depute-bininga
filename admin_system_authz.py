...
_PERMISSION_RULES = {
    ...
    ("GET", "/api/crm"): "crm.read",
    ("GET", "/api/crm/export"): "crm.export",
    ("GET", "/api/backups"): "backup.read",
    ("GET", "/api/logs"): "logs.read",
    ("GET", "/api/security"): "security.read",
    ("GET", "/api/security/bouclier"): "security.read",
    ("GET", "/api/monitoring/summary"): "monitoring.read",
    ("GET", "/api/monitoring/alerts"): "monitoring.read",
    ("GET", "/api/monitoring/endpoints"): "monitoring.read",
    ("GET", "/api/monitoring/requests"): "monitoring.read",
    ("GET", "/api/monitoring/exceptions"): "monitoring.read",
}

_PREFIX_PERMISSION_RULES = (
    ("POST", "/api/crm/", "crm.write"),
    ("POST", "/api/backups/", "backup.create"),
    ("POST", "/api/security/", "security.manage"),
    ("POST", "/api/monitoring/", "monitoring.manage"),
)

_STATELESS_POSTS_WITHOUT_DB = {
    "/api/login", "/api/logout", "/api/chat", "/api/track-visit", "/api/test/reset",
}
...
def _set_response_body(handler, body: bytes) -> None:
    ...
    handler._response_headers = [(key, value) for key, value in handler._response_headers if str(key).lower() != "content-length"]
