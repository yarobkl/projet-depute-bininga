...
def test_granular_permissions_are_server_authoritative():
    policy = read("admin_permissions.py"); authz = read("admin_system_authz.py"); access = read("admin_access_model.py")
    ...
    assert "admin_permissions.has_permission" in authz and '"PERMISSION_DENIED"' in authz
...
def test_database_resilience_fails_closed_with_probe_and_circuit_breaker():
    resilience = read("db_resilience.py"); authz = read("admin_system_authz.py")
    ...
    assert "db_resilience.can_persist" in authz and '"PERSISTENCE_REQUIRED"' in authz


def test_compromised_account_has_session_inventory_revocation_and_owner_2fa_gate():
    incident = read("account_incident_response.py"); pipeline = read("admin_request_pipeline.py")
    assert '"/api/auth/sessions"' in incident and '"/api/auth/revoke-sessions"' in incident
    assert "bearer tokens" in incident.lower()
    assert "OWNER_2FA_REQUIRED" in incident and "totp_secret" in incident
...
def test_restore_is_verified_transactional_and_has_safety_backup():
    ...
    assert "backup_bininga.verify_backup" in restore and "_create_safety_backup" in restore
    assert "conn.commit()" in restore and "conn.rollback()" in restore and "EXCLUDED_STORE_KEYS" in restore
...
def test_advanced_security_ui_exists_without_becoming_authoritative():
    ...
    assert "remain authoritative" in ui
