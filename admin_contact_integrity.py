...
This module hardens contact/CRM handlers without rewriting the large server.py.
"""
...
def guard_request(server, handler) -> bool:
    ...
    if path in {"/api/contacts/clear", "/api/reset"}:
        if not owner_policy.is_owner_session(server, session):
            server.audit_log("AUTHZ_REJECT", handler.client_address[0], f"Opération destructive refusée sur {path} pour {session.get('username', '?')}")
